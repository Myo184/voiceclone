...
def loading_flow_html():
    ...
    <div class="loading-flow">
      ...
      <div class="loading-steps"><span>1. Voice analyse</span><span>2. Clone</span><span>3. MP3 + SRT</span></div>
    </div>
    """
...
def cleanup_generated_outputs(keep_recent=MAX_SAVED_JOBS):
    """Keep only the newest completed MP3/SRT jobs to avoid filling Colab storage."""
    try:
        for prefix, suffix in (("Long_Voice_", ".mp3"), ("Long_Subtitle_", ".srt")):
            files = [
...
def generate_vip_long(vip_key, device_fingerprint, text, control_instruction, reference_audio, use_reference_transcript, reference_text, clone_strength, progress=gr.Progress()):
    ...
    if failed_chunk is not None:
        ...
        record_voice_usage(vip_key, request_chars, "failed_refunded", "MP3+SRT", time.time() - start_all, failed_error or "chunk generation failed", job_id)
    ...
    if not audio_segments or len(subtitles) != total:
        release_vip_quota(vip_key.strip(), request_chars)
        record_voice_usage(vip_key, request_chars, "failed_refunded", "MP3+SRT", time.time() - start_all, "Audio/SRT completeness check failed", job_id)
        _, _, total2, used2, remaining2 = verify_vip_license(vip_key, device_fingerprint)
        return (
            None, None, None,
    ...
    output_srt_path = os.path.join(OUTPUT_DIR, f"Long_Subtitle_{ts}.srt")
    ...
    try:
        ...
        release_vip_quota(vip_key.strip(), request_chars)
        record_voice_usage(vip_key, request_chars, "failed_refunded", "MP3+SRT", time.time() - start_all, f"{type(exc).__name__}: {exc}", job_id)
    ...
    record_voice_usage(vip_key, request_chars, "success", "MP3+SRT", generation_sec, "", job_id)
...
with gr.Blocks(title="YF TTS · Burmese AI Voice Studio", theme=APP_THEME, css=APP_CSS) as demo:
    ...
    with gr.Column(visible=False, elem_classes=["wizard-stage", "stage-card", "panel"]) as voice_step:
        ...
        gen_btn = gr.Button("✨ MP3 + SRT စတင်ထုတ်လုပ်မည်", variant="primary", elem_id="generate-btn")
    ...
    with gr.Column(visible=False, elem_classes=["wizard-stage", "stage-card", "panel"]) as result_step:
        ...
        <h3>သင့်အသံဖိုင်ကို ပြင်ဆင်နေပါသည်</h3><p>ပြီးသွားလျှင် MP3 နှင့် SRT ကို ဒီနေရာကနေ download လုပ်နိုင်ပါသည်။</p></div>
        """)
