...
def _bind_or_check_device(collection, record, device_fingerprint):
    ...
    if not _device_lock_is_enabled(collection):
        collection.update_one(
            {"_id": record["_id"]},
            {"$set": {"last_device_used_at": datetime.datetime.now(datetime.timezone.utc)}},
        )
        return True, "ℹ️ Admin မှ Device Lock ကို ပိတ်ထားပါသည်။"
    ...
    try:
        ...
        if unlimited:
            ...
            if device_lock_enabled:
                unlimited_query["device_id_hash"] = token_hash
        ...
        if device_lock_enabled:
            quota_query["device_id_hash"] = token_hash
        updated = collection.find_one_and_update(
            quota_query,
            {
                "$inc": {
